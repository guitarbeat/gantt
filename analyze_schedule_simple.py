...
def analyze_schedule_balance(csv_file):
    ...
    heaviest_quarters = sorted(workload_by_quarter.items(), key=lambda x: x[1], reverse=True)[:3]
    print(f"   - Heaviest quarters: {[f'{q} ({days} days)' for q, days in heaviest_quarters]}")
    
    # Find the lightest quarters
    lightest_quarters = sorted(workload_by_quarter.items(), key=lambda x: x[1])[:3]
    print(f"   - Lightest quarters: {[f'{q} ({days} days)' for q, days in lightest_quarters]}")
