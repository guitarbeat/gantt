        ...
        if pd.notna(row['Start Date']) and pd.notna(row['Due Date']):
            ...
            if start_quarter == end_quarter:
                ...
                if row['Start Date'] <= q1_end and row['Due Date'] >= q1_end:
                    workload_by_quarter[start_quarter] += (
                        q1_end - row['Start Date']).days + 1
                if row['Start Date'] <= q2_end and row['Due Date'] >= q2_end:
                    q2_start = max(row['Start Date'], datetime(
                        row['Start Date'].year, 4, 1))
                    workload_by_quarter[f"{row['Start Date'].year}-Q2"] += (
                        q2_end - q2_start).days + 1
                if row['Start Date'] <= q3_end and row['Due Date'] >= q3_end:
                    q3_start = max(row['Start Date'], datetime(
                        row['Start Date'].year, 7, 1))
                    workload_by_quarter[f"{row['Start Date'].year}-Q3"] += (
                        q3_end - q3_start).days + 1
                if row['Start Date'] <= q4_end and row['Due Date'] >= q4_end:
                    q4_start = max(row['Start Date'], datetime(
                        row['Start Date'].year, 10, 1))
                    workload_by_quarter[f"{row['Start Date'].year}-Q4"] += (
                        q4_end - q4_start).days + 1
