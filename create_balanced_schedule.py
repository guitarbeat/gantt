...
def create_balanced_schedule(input_file, output_file):
    ...
    for task in tasks:
        ...
        if task['Category'] == 'DISSERTATION':
            if task['Task ID'] == 'BI':  # Draft Introduction
                ...
                # Start immediately after defense
                new_task['Start Date'] = datetime(2027, 6, 18)
                new_task['Due Date'] = datetime(
                    2027, 7, 15)    # Extend revision time
            elif task['Task ID'] == 'BU':  # Submit dissertation
                new_task['Start Date'] = datetime(
                    2027, 7, 16)  # Start after revisions
                new_task['Due Date'] = datetime(
                    2027, 7, 31)    # Earlier submission
        ...
        # * Rebalance imaging work - spread out heavy periods
            ...
            if task['Task ID'] in ['AJ', 'AK', 'AM']:  # Parallel development tasks
                ...
                if task['Task ID'] == 'AJ':  # Develop U-Net pipeline
                    new_task['Start Date'] = datetime(
                        2026, 3, 15)  # Start earlier
                    new_task['Due Date'] = datetime(
                        2026, 5, 15)    # Extend duration
                elif task['Task ID'] == 'AK':  # Optimize imaging systems
                    new_task['Start Date'] = datetime(
                        2026, 3, 20)  # Start earlier
                    new_task['Due Date'] = datetime(
                        2026, 6, 15)    # Extend duration
                elif task['Task ID'] == 'AM':  # Order enhanced AAV
                    new_task['Start Date'] = datetime(
                        2026, 3, 25)  # Start earlier
                    new_task['Due Date'] = datetime(
                        2026, 6, 15)    # Extend duration
            ...
            # * Spread out stroke imaging sessions
                ...
                if task['Task ID'] == 'AT':  # Acute-phase imaging
                    new_task['Start Date'] = datetime(
                        2026, 6, 20)  # Slightly later
                    new_task['Due Date'] = datetime(
                        2026, 6, 25)    # Extend duration
                elif task['Task ID'] == 'AU':  # Transition-phase imaging
                    new_task['Start Date'] = datetime(
                        2026, 7, 10)  # More spacing
                    new_task['Due Date'] = datetime(
                        2026, 7, 15)    # Extend duration
                elif task['Task ID'] == 'AV':  # Stabilization-phase imaging
                    new_task['Start Date'] = datetime(
                        2026, 8, 5)   # More spacing
                    new_task['Due Date'] = datetime(
                        2026, 8, 10)    # Extend duration
                elif task['Task ID'] == 'AW':  # Extended chronic imaging
                    new_task['Start Date'] = datetime(
                        2026, 9, 1)   # More spacing
                    new_task['Due Date'] = datetime(
                        2026, 9, 5)     # Extend duration
        ...
        # * Extend some short tasks to reduce daily intensity
            ...
            new_task['Duration'] = max(
                5, task['Duration'] + 2)  # Add 2 days minimum
            new_task['Due Date'] = new_task['Start Date'] + \
                timedelta(days=new_task['Duration'] - 1)
    ...
    with open(output_file, 'w', newline='') as f:
        fieldnames = ['Task ID', 'Task Name', 'Parent Task ID', 'Category',
                      'Start Date', 'Due Date', 'Dependencies', 'Description']
    ...
    print(
        f"Added {len(buffer_tasks)} buffer tasks and {len(parallel_tasks)} parallel tasks")
