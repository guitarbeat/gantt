...
func (mle *MultiDayLayoutEngine) LayoutMultiDayTasks(tasks []*shared.Task) []*TaskBar {
...
func (mle *MultiDayLayoutEngine) groupOverlappingTasks(tasks []*shared.Task) []*TaskGroup {
...
func (mle *MultiDayLayoutEngine) tasksOverlap(group *TaskGroup, task *shared.Task) bool {
...
func (mle *MultiDayLayoutEngine) tasksOverlapDirect(task1, task2 *shared.Task) bool {
...
func (mle *MultiDayLayoutEngine) findAvailableRow(task *shared.Task, rowEndTimes []time.Time) int {
	for i, endTime := range rowEndTimes {
		if task.StartDate.After(endTime) || task.StartDate.Equal(endTime) {
			return i
		}
...
func (mle *MultiDayLayoutEngine) createTaskBar(task *shared.Task, row, totalRows int) *TaskBar {
...
func (mle *MultiDayLayoutEngine) isTaskContinuation(task *shared.Task) bool {
...
func (mle *MultiDayLayoutEngine) isTaskStart(task *shared.Task) bool {
	// Check if task starts on or after calendar start
	return !task.StartDate.Before(mle.calendarStart)
}

// isTaskEnd checks if this is the end of a multi-day task
func (mle *MultiDayLayoutEngine) isTaskEnd(task *shared.Task) bool {
...
func (mle *MultiDayLayoutEngine) hasMonthBoundary(task *shared.Task) bool {
