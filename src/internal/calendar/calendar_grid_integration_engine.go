...
func (cgi *CalendarGridIntegration) ProcessTasksWithSmartStacking(tasks []*shared.Task) (*IntegratedLayoutResult, error) {
	// Step 1: Detect overlaps and conflicts
	overlapAnalysis := cgi.stackingEngine.overlapDetector.DetectOverlaps(tasks)
	...
	priorityManagementEngine := NewPriorityManagementEngine(
		cgi.stackingEngine.conflictCategorizer,
		cgi.stackingEngine,
	)
	...
	stackingResult := cgi.stackingEngine.StackTasks(tasks, stackingContext)
	
	// Step 5: Apply vertical stacking
	verticalStackingResult := cgi.stackingEngine.StackTasksVertically(tasks, stackingContext)
...
func (cgi *CalendarGridIntegration) calculateVisualWeight(task *shared.Task, priority *TaskPriority) float64 {
...
func (cgi *CalendarGridIntegration) calculateProminenceScore(task *shared.Task, priority *TaskPriority, visualWeight float64) float64 {
...
func (cgi *CalendarGridIntegration) calculateTaskHeight(task *shared.Task, visualWeight float64) float64 {
	// Base height
...
func (cgi *CalendarGridIntegration) isTaskContinuation(task *shared.Task) bool {
	return task.StartDate.Before(cgi.gridConfig.CalendarStart)
}

// isTaskStart checks if this task starts on this day
func (cgi *CalendarGridIntegration) isTaskStart(task *shared.Task) bool {
...
func (cgi *CalendarGridIntegration) isTaskEnd(task *shared.Task) bool {
...
func (cgi *CalendarGridIntegration) hasMonthBoundary(task *shared.Task) bool {
...
func (cgi *CalendarGridIntegration) ProcessTasksWithValidation(tasks []*shared.Task) (*MultiDayLayoutResult, error) {
...
func (cgi *CalendarGridIntegration) filterValidTasks(tasks []*shared.Task, validationErrors []shared.DataValidationError) []*shared.Task {
