...
// ConflictManagementEngine handles both conflict categorization and resolution
type ConflictManagementEngine struct {
...
// NewConflictManagementEngine creates a new conflict management engine
func NewConflictManagementEngine(
	overlapDetector *OverlapDetector,
	taskPrioritizationEngine *TaskPrioritizationEngine,
	stackingEngine *StackingEngine,
) *ConflictManagementEngine {
	engine := &ConflictManagementEngine{
		overlapDetector:         overlapDetector,
...
func (cme *ConflictManagementEngine) CategorizeConflicts(analysis *OverlapAnalysis) *ConflictAnalysis {
	...
	for _, group := range analysis.OverlapGroups {
		for _, overlap := range group.Overlaps {
			...
			task1, task2 := cme.getTasksFromOverlap(overlap, group.Tasks)
			if task1 == nil || task2 == nil {
				continue
			}

			// Categorize the conflict
			categorizedConflict := cme.categorizeConflict(overlap, task1, task2)
	...
	conflictAnalysis.RiskAssessment = cme.generateRiskAssessment(conflictAnalysis)
	conflictAnalysis.Recommendations = cme.generateRecommendations(conflictAnalysis)

	return conflictAnalysis
}

// ResolveConflicts performs comprehensive conflict resolution
func (cme *ConflictManagementEngine) ResolveConflicts(tasks []*shared.Task, context *PriorityContext) *ConflictResolutionResult {
	// Use the conflict resolution engine
	conflictResolutionEngine := NewConflictResolutionEngine(
		cme.taskPrioritizationEngine,
		cme.stackingEngine,
	)
	
	return conflictResolutionEngine.ResolveConflicts(tasks, context)
}
...
func (cre *ConflictResolutionEngine) ResolveConflicts(tasks []*shared.Task, context *PriorityContext) *ConflictResolutionResult {
	// Simplified implementation - in real implementation would be more sophisticated
...
func (cme *ConflictManagementEngine) initializeDefaultRules() {
	cme.rules = []ConflictRule{
		{
			Name:        "Identical Schedule Conflict",
			Description: "Tasks have identical start and end dates",
			Condition: func(overlap *TaskOverlap, task1, task2 *shared.Task) bool {
...
func (cme *ConflictManagementEngine) AddCustomRule(rule ConflictRule) {
	cme.rules = append(cme.rules, rule)
	// Sort rules by priority (higher priority first)
	sort.Slice(cme.rules, func(i, j int) bool {
		return cme.rules[i].Priority > cme.rules[j].Priority
	})
}

// categorizeConflict categorizes a single conflict
func (cme *ConflictManagementEngine) categorizeConflict(overlap *TaskOverlap, task1, task2 *shared.Task) *CategorizedConflict {
	// Find the best matching rule
	var bestRule *ConflictRule
	highestPriority := 0

	for _, rule := range cme.rules {
	...
	categorizedConflict := &CategorizedConflict{
		...
		SubCategory: cme.determineSubCategory(overlap, task1, task2),
		RootCause:   cme.determineRootCause(overlap, task1, task2, bestRule),
		Impact:      cme.assessImpact(overlap, task1, task2),
		Resolution:  cme.generateResolution(overlap, task1, task2, bestRule),
		RiskLevel:   cme.assessRiskLevel(overlap, task1, task2),
		Urgency:     cme.assessUrgency(overlap, task1, task2),
		Complexity:  cme.assessComplexity(overlap, task1, task2),
	}

	// Generate alternative resolutions
	categorizedConflict.AlternativeResolutions = cme.generateAlternativeResolutions(overlap, task1, task2, bestRule)

	return categorizedConflict
}

// determineSubCategory determines a more specific subcategory
func (cme *ConflictManagementEngine) determineSubCategory(overlap *TaskOverlap, task1, task2 *shared.Task) string {
...
func (cme *ConflictManagementEngine) determineRootCause(overlap *TaskOverlap, task1, task2 *shared.Task, rule *ConflictRule) string {
...
func (cme *ConflictManagementEngine) assessImpact(overlap *TaskOverlap, task1, task2 *shared.Task) string {
	// Calculate impact based on multiple factors
	impactScore := 0

	// Severity impact
	impactScore += cme.severityWeights[overlap.Severity]
...
func (cme *ConflictManagementEngine) generateResolution(overlap *TaskOverlap, task1, task2 *shared.Task, rule *ConflictRule) ConflictResolution {
	switch rule.Category {
	case CategoryScheduleConflict:
...
func (cme *ConflictManagementEngine) generateAlternativeResolutions(overlap *TaskOverlap, task1, task2 *shared.Task, rule *ConflictRule) []ConflictResolution {
...
func (cme *ConflictManagementEngine) assessRiskLevel(overlap *TaskOverlap, task1, task2 *shared.Task) string {
	riskScore := 0

	// Severity risk
	riskScore += cme.severityWeights[overlap.Severity]
...
func (cme *ConflictManagementEngine) assessUrgency(overlap *TaskOverlap, task1, task2 *shared.Task) string {
	urgencyScore := 0

	// Severity urgency
	urgencyScore += cme.severityWeights[overlap.Severity]
...
func (cme *ConflictManagementEngine) assessComplexity(overlap *TaskOverlap, task1, task2 *shared.Task) string {
	complexityScore := 0
...
func (cme *ConflictManagementEngine) getTasksFromOverlap(overlap *TaskOverlap, tasks []*shared.Task) (*shared.Task, *shared.Task) {
	var task1, task2 *shared.Task
...
func (cme *ConflictManagementEngine) generateRiskAssessment(analysis *ConflictAnalysis) string {
...
func (cme *ConflictManagementEngine) generateRecommendations(analysis *ConflictAnalysis) []string {
...
// Additional types and methods from conflict resolution engine
