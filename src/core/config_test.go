...
func TestDefaultConfig(t *testing.T) {
	...
	if cfg.OutputDir == "" {
		t.Error("DefaultConfig() OutputDir should not be empty")
	}

	if cfg.OutputDir != "generated" {
		t.Errorf("DefaultConfig() OutputDir = %s, want 'generated'", cfg.OutputDir)
	}

	// Test that Layout is initialized
	if cfg.Layout.LaTeX.Typography.HyphenPenalty == 0 {
		t.Error("DefaultConfig() should initialize Typography.HyphenPenalty")
	}
}
...
func TestFilterUniqueModules(t *testing.T) {
	...
	if len(filtered) != 2 {
		t.Errorf("FilterUniqueModules() returned %d modules, want 2", len(filtered))
	}

	// Check that we kept the first occurrence
	if filtered[0].Tpl != "template1" {
		t.Errorf("FilterUniqueModules() first module = %s, want 'template1'", filtered[0].Tpl)
	}
