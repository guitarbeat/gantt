...
type MultiDayLayoutResult struct {
	TaskBars         []*TaskBar
	ValidationResult []core.DataValidationError
	LayoutIssues     []string
	TaskCount        int
	ProcessedCount   int
}
...
func NewLayoutEngine(config *GridConfig) *LayoutEngine {
	...
	dateValidator := core.NewDateValidator()
...
func (le *LayoutEngine) LayoutMultiDayTasks(tasks []*core.Task) []*TaskBar {
...
func (le *LayoutEngine) groupOverlappingTasks(tasks []*core.Task) []*TaskGroup {
...
func (le *LayoutEngine) tasksOverlap(group *TaskGroup, task *core.Task) bool {
...
func (le *LayoutEngine) tasksOverlapDirect(task1, task2 *core.Task) bool {
...
func (le *LayoutEngine) findAvailableRow(task *core.Task, rowEndTimes []time.Time) int {
...
func (le *LayoutEngine) createTaskBar(task *core.Task, row, totalRows int) *TaskBar {
	...
	category := core.GetCategory(task.Category)
...
func (le *LayoutEngine) isTaskContinuation(task *core.Task) bool {
...
func (le *LayoutEngine) isTaskStart(task *core.Task) bool {
	// Check if task starts on or after calendar start
	return !task.StartDate.Before(le.calendarStart)
}

// isTaskEnd checks if this is the end of a multi-day task
func (le *LayoutEngine) isTaskEnd(task *core.Task) bool {
...
func (le *LayoutEngine) hasMonthBoundary(task *core.Task) bool {
...
func (le *LayoutEngine) ProcessTasksWithSmartStacking(tasks []*core.Task) (*IntegratedLayoutResult, error) {
...
func (le *LayoutEngine) calculateVisualWeight(task *core.Task, priority *TaskPriority) float64 {
...
func (le *LayoutEngine) calculateProminenceScore(task *core.Task, priority *TaskPriority, visualWeight float64) float64 {
...
func (le *LayoutEngine) calculateTaskHeight(task *core.Task, visualWeight float64) float64 {
	// Base height
...
func (le *LayoutEngine) ProcessTasksWithValidation(tasks []*core.Task) (*MultiDayLayoutResult, error) {
...
func (le *LayoutEngine) filterValidTasks(tasks []*core.Task, validationErrors []core.DataValidationError) []*core.Task {
...
func (se *SpatialEngine) DetectOverlaps(tasks []*core.Task) *OverlapAnalysis {
...
func (se *SpatialEngine) groupOverlappingTasks(tasks []*core.Task) []*TaskGroup {
...
// tasksOverlap checks if a task overlaps with a group
func (se *SpatialEngine) tasksOverlap(group *TaskGroup, task *core.Task) bool {
	for _, groupTask := range group.Tasks {
...
// tasksOverlapDirect checks if two tasks overlap directly
func (se *SpatialEngine) tasksOverlapDirect(task1, task2 *core.Task) bool {
	// Tasks overlap if one starts before the other ends
	return !task1.StartDate.After(task2.EndDate) && !task2.StartDate.After(task1.EndDate)
}

// detectGroupOverlaps detects all overlaps within a group of tasks
func (se *SpatialEngine) detectGroupOverlaps(tasks []*core.Task) []*TaskOverlap {
...
func (se *SpatialEngine) analyzeTaskOverlap(task1, task2 *core.Task) *TaskOverlap {
...
func (se *SpatialEngine) determineOverlapType(task1, task2 *core.Task, overlapStart, overlapEnd time.Time) OverlapType {
...
func (se *SpatialEngine) calculateOverlapSeverity(task1, task2 *core.Task, overlapType OverlapType, duration time.Duration) OverlapSeverity {
...
func (se *SpatialEngine) calculateOverlapPercentage(task1, task2 *core.Task, overlapDuration time.Duration) float64 {
...
func (se *SpatialEngine) generateConflictInfo(task1, task2 *core.Task, overlapType OverlapType, severity OverlapSeverity) (string, string) {
...
func (se *SpatialEngine) PositionTasks(tasks []*core.Task, existingBars []*IntegratedTaskBar) (*PositioningResult, error) {
...
func (se *SpatialEngine) createIntegratedTaskBars(tasks []*core.Task, context *PositioningContext) []*IntegratedTaskBar {
...
func (se *SpatialEngine) calculateInitialYPosition(task *core.Task, context *PositioningContext) float64 {
...
func (se *SpatialEngine) isTaskContinuation(task *core.Task, context *PositioningContext) bool {
	return task.StartDate.Before(context.CalendarStart)
}

func (se *SpatialEngine) isTaskStart(task *core.Task, context *PositioningContext) bool {
	return task.StartDate.Equal(context.CalendarStart) || task.StartDate.After(context.CalendarStart)
}

func (se *SpatialEngine) isTaskEnd(task *core.Task, context *PositioningContext) bool {
	return task.EndDate.Equal(context.CalendarEnd) || task.EndDate.Before(context.CalendarEnd)
}

func (se *SpatialEngine) hasMonthBoundary(task *core.Task, context *PositioningContext) bool {
	startMonth := task.StartDate.Month()
	endMonth := task.EndDate.Month()
	return startMonth != endMonth
}
...
func (se *SpatialEngine) calculateTaskDensity(tasks []*core.Task) float64 {
	if len(tasks) == 0 {
		return 0.0
	}
