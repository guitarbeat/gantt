#!/usr/bin/env python3
"""
Create a truly balanced PhD schedule with work distributed evenly throughout the timeline.
Target: ~150-200 task-days per quarter for manageable workload.
"""
...
def create_final_balanced_schedule(input_file, output_file):
    """Create a truly balanced schedule with systematic redistribution."""
    ...
    # Target: ~150-180 task-days per quarter (manageable workload)
    target_quarterly_workload = 170
    
    improved_tasks = []
    
    # * Strategy: Redistribute work across 8 quarters (2025-Q3 to 2027-Q2)
    # Each quarter should have ~150-180 task-days
    
    # * Phase 1: Redistribute dissertation work across 6 quarters (2026-Q1 to 2027-Q2)
    dissertation_tasks = [t for t in tasks if t['Category'] == 'DISSERTATION']
    other_tasks = [t for t in tasks if t['Category'] != 'DISSERTATION']
    
    # Spread dissertation work evenly from 2026-Q1 to 2027-Q2
    for task in dissertation_tasks:
        new_task = task.copy()
        
        if task['Task ID'] == 'BI':  # Draft Introduction
            new_task['Start Date'] = datetime(2026, 1, 1)
            new_task['Due Date'] = datetime(2026, 4, 30)
        elif task['Task ID'] == 'BJ':  # Draft Aim 1 chapter
            new_task['Start Date'] = datetime(2026, 2, 1)
            new_task['Due Date'] = datetime(2026, 5, 31)
            ...
            new_task['Due Date'] = datetime(2026, 8, 31)
            ...
            new_task['Start Date'] = datetime(2027, 1, 15)
            new_task['Due Date'] = datetime(2027, 1, 17)
        elif task['Task ID'] == 'BT':  # Revise dissertation
            new_task['Start Date'] = datetime(2027, 1, 18)
            new_task['Due Date'] = datetime(2027, 3, 31)
        elif task['Task ID'] == 'BU':  # Submit dissertation
            new_task['Start Date'] = datetime(2027, 4, 1)
            new_task['Due Date'] = datetime(2027, 4, 15)
        
        improved_tasks.append(new_task)
    
    # * Phase 2: Redistribute imaging work across 6 quarters (2025-Q4 to 2026-Q3)
    ...
    for task in imaging_tasks:
        ...
        # * Laser setup in 2025-Q4
        if task['Task ID'] in ['H', 'I', 'J', 'K', 'L']:
            if task['Task ID'] == 'H':  # Align seed laser
                new_task['Start Date'] = datetime(2025, 10, 1)
                new_task['Due Date'] = datetime(2025, 10, 15)
            elif task['Task ID'] == 'I':  # Align amplifier
                new_task['Start Date'] = datetime(2025, 10, 16)
                new_task['Due Date'] = datetime(2025, 11, 15)
            elif task['Task ID'] == 'J':  # Check pulse compression
                new_task['Start Date'] = datetime(2025, 11, 16)
                new_task['Due Date'] = datetime(2025, 12, 5)
            elif task['Task ID'] == 'K':  # Calibrate microscope
                new_task['Start Date'] = datetime(2025, 12, 6)
                new_task['Due Date'] = datetime(2025, 12, 20)
            elif task['Task ID'] == 'L':  # Laser system ready
                new_task['Start Date'] = datetime(2025, 12, 21)
                new_task['Due Date'] = datetime(2025, 12, 31)
        
        # * Pilot imaging in 2026-Q1
        elif task['Task ID'] in ['M', 'P', 'Q', 'W', 'Z', 'AA', 'AB', 'AC', 'AD', 'AE', 'AF', 'AG', 'AH']:
            if task['Task ID'] == 'M':  # Plan imaging cohort
                new_task['Start Date'] = datetime(2026, 1, 1)
                new_task['Due Date'] = datetime(2026, 1, 15)
            elif task['Task ID'] == 'P':  # Design AAV vectors
                new_task['Start Date'] = datetime(2026, 1, 16)
                new_task['Due Date'] = datetime(2026, 2, 15)
            elif task['Task ID'] == 'Q':  # AAV vectors ready
                new_task['Start Date'] = datetime(2026, 2, 16)
                new_task['Due Date'] = datetime(2026, 2, 28)
            elif task['Task ID'] == 'W':  # Cranial window surgery #1
                new_task['Start Date'] = datetime(2026, 3, 1)
                new_task['Due Date'] = datetime(2026, 3, 5)
            elif task['Task ID'] == 'Z':  # Post-op recovery #1
                new_task['Start Date'] = datetime(2026, 3, 6)
                new_task['Due Date'] = datetime(2026, 3, 12)
            elif task['Task ID'] == 'AA':  # Cranial window surgery #2
                new_task['Start Date'] = datetime(2026, 3, 13)
                new_task['Due Date'] = datetime(2026, 3, 17)
            elif task['Task ID'] == 'AB':  # Post-op recovery #2
                new_task['Start Date'] = datetime(2026, 3, 18)
                new_task['Due Date'] = datetime(2026, 3, 24)
            elif task['Task ID'] == 'AC':  # Cranial window surgery #3
                new_task['Start Date'] = datetime(2026, 3, 25)
                new_task['Due Date'] = datetime(2026, 3, 29)
            elif task['Task ID'] == 'AD':  # Post-op recovery #3
                new_task['Start Date'] = datetime(2026, 3, 30)
                new_task['Due Date'] = datetime(2026, 4, 5)
            elif task['Task ID'] == 'AE':  # Pilot imaging session #1
                new_task['Start Date'] = datetime(2026, 4, 6)
                new_task['Due Date'] = datetime(2026, 4, 10)
            elif task['Task ID'] == 'AF':  # Pilot imaging session #2
                new_task['Start Date'] = datetime(2026, 4, 11)
                new_task['Due Date'] = datetime(2026, 4, 15)
            elif task['Task ID'] == 'AG':  # Pilot imaging session #3
                new_task['Start Date'] = datetime(2026, 4, 16)
                new_task['Due Date'] = datetime(2026, 4, 20)
            elif task['Task ID'] == 'AH':  # Pilot datasets complete
                new_task['Start Date'] = datetime(2026, 4, 21)
                new_task['Due Date'] = datetime(2026, 4, 25)
        
        # * Development work in 2026-Q2
        elif task['Task ID'] in ['AI', 'AJ', 'AK', 'AM', 'AN', 'AO']:
            if task['Task ID'] == 'AI':  # Process pilot data
                new_task['Start Date'] = datetime(2026, 4, 26)
                new_task['Due Date'] = datetime(2026, 5, 15)
            elif task['Task ID'] == 'AJ':  # Develop U-Net pipeline
                new_task['Start Date'] = datetime(2026, 5, 1)
                new_task['Due Date'] = datetime(2026, 6, 30)
            elif task['Task ID'] == 'AK':  # Optimize imaging systems
                new_task['Start Date'] = datetime(2026, 5, 16)
                new_task['Due Date'] = datetime(2026, 7, 15)
            elif task['Task ID'] == 'AM':  # Order enhanced AAV
                new_task['Start Date'] = datetime(2026, 6, 1)
                new_task['Due Date'] = datetime(2026, 7, 31)
            elif task['Task ID'] == 'AN':  # Enhanced AAV delivered
                new_task['Start Date'] = datetime(2026, 8, 1)
                new_task['Due Date'] = datetime(2026, 8, 5)
            elif task['Task ID'] == 'AO':  # Compare labeling methods
                new_task['Start Date'] = datetime(2026, 8, 6)
                new_task['Due Date'] = datetime(2026, 9, 30)
        
        # * Stroke imaging in 2026-Q3
    ...
    # * Phase 3: Redistribute publication work across 4 quarters (2026-Q1 to 2026-Q4)
    ...
    for task in publication_tasks:
        ...
        if task['Task ID'] == 'AP':  # Draft methodology paper
            ...
            new_task['Start Date'] = datetime(2026, 6, 1)
            new_task['Due Date'] = datetime(2026, 6, 15)
    ...
    # * Phase 5: Add strategic buffer and parallel tasks to fill light periods
    buffer_tasks = [
        {
            'Task ID': 'BUF1',
        ...
        },
        {
            ...
            'Parent Task ID': '',
            'Category': 'DISSERTATION',
            'Start Date': datetime(2026, 10, 1),
            'Due Date': datetime(2026, 10, 31),
            'Dependencies': 'BM',
    ...
    parallel_tasks = [
        ...
        {
            'Task ID': 'PAR5',
            'Task Name': 'Develop data visualization tools',
            'Parent Task ID': '',
            'Category': 'IMAGING',
            'Start Date': datetime(2026, 6, 1),
            'Due Date': datetime(2026, 7, 31),
            'Dependencies': 'AI',
            'Description': 'Develop data visualization and analysis tools in parallel with data collection.'
        }
    ]
    ...
    print(f"Final balanced schedule written to {output_file}")
...
if __name__ == "__main__":
    create_final_balanced_schedule(
