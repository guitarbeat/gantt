...
func ApplySpanningTasksToMonth(month *Month, tasks []SpanningTask) {
	// Create a lookup map for days in the month for O(1) access
	// Key: day number (1-31), Value: pointer to Day
	dayLookup := make(map[int]*Day)

	for _, week := range month.Weeks {
		for i := range week.Days {
			// Only index days belonging to this month
			if week.Days[i].Time.Month() == month.Month {
				dayLookup[week.Days[i].Time.Day()] = &week.Days[i]
			}
		}
	}
	...
	for taskIndex, task := range tasks {
		// Optimization: Calculate intersection of task duration and month duration
		// This avoids iterating through days that are outside the current month

		// Determine effective start date (max of task start and month start)
		effectiveStart := task.StartDate
		if effectiveStart.Before(monthStart) {
			effectiveStart = monthStart
		}

		// Determine effective end date (min of task end and month end)
		effectiveEnd := task.EndDate
		if effectiveEnd.After(monthEnd) {
			effectiveEnd = monthEnd
		}

		// If effective start is after effective end, there is no overlap
		if effectiveStart.After(effectiveEnd) {
			continue
		}

		// Create a copy of the task to avoid pointer issues
		// We do this once per task instead of inside the loop
		taskCopy := tasks[taskIndex]

		// Iterate only through the days in the intersection
		current := effectiveStart
		for !current.After(effectiveEnd) {
			dayNum := current.Day()

			// Use the lookup map to find the day directly
			if day, found := dayLookup[dayNum]; found {
				day.Tasks = append(day.Tasks, &taskCopy)
			}
