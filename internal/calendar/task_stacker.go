...
func (ts *TaskStacker) assignTaskToTrack(task *SpanningTask, track int) {
	current := ts.normalizeDate(task.StartDate)
	end := ts.normalizeDate(task.EndDate)
...
// dateKey creates a unique key for a date (YYYYMMDD int)
...
// kept for backward compatibility if needed, but internally unused for core loops
