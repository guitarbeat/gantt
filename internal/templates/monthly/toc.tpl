...
\hline
    {{- range $i, $task := index $.Body.TaskIndex $phase}}
        {{- $taskName := $task.Name }}
        {{- $taskIcon := "" }}
        {{- if $task.IsMilestone}}{{- $taskIcon = "\\BeginAccSupp{method=pdfstringdef,Unicode,ActualText={Milestone: }}\\textbf{$\\star$}\\EndAccSupp{}" }}{{- $taskName = printf "\\textbf{%s}" $taskName}}{{- end}}
        {{- if eq ($task.Status | lower) "completed"}}{{- $taskIcon = "\\BeginAccSupp{method=pdfstringdef,Unicode,ActualText={Completed: }}\\textbf{$\\checkmark$}\\EndAccSupp{}" }}{{- $taskName = printf "\\textcolor{gray}{%s}" $taskName}}{{- end}}
