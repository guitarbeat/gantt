...
\hline
    {{- range $i, $task := index $.Body.TaskIndex $phase}}
        {{- $taskName := $task.Name }}
        {{- $taskIcon := "" }}
        {{- if $task.IsMilestone}}{{- $taskIcon = "\\BeginAccSupp{ActualText={Milestone: }}\\textbf{$\\star$}\\EndAccSupp{}" }}{{- $taskName = printf "\\textbf{%s}" $taskName}}{{- end}}
        {{- if eq ($task.Status | lower) "completed"}}{{- $taskIcon = "\\BeginAccSupp{ActualText={Completed: }}\\checkmark\\EndAccSupp{}" }}{{- $taskName = printf "\\textcolor{gray}{%s}" $taskName}}{{- end}}
